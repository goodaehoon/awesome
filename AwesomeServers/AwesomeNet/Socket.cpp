...
SOCKET Socket::Create(int type, int protocol)
{
	...
	if (fd == INVALID_SOCKET)
		LOG_CORE("fail socket() err(%d)\n", WSAGetLastError());
#else
	fd = socket(AF_INET, type, 0);
	if (fd == -1)
		LOG_CORE("fail socket() err(%d) : %s\n", errno, strerror(errno));
#endif
...
SOCKET Socket::Accept(sockaddr_in* addr)
{
	...
	if (s == -1)
	{
		LOG_CORE("fail accept() error(%d): %s\n", errno, strerror(errno));
		return -1;
	}
...
bool Socket::Nonblock()
{
	...
	if (flags == -1)
	{
		LOG_CORE("fail fcntl(F_GETFL) err(%d) : %s\n", errno, strerror(errno));
		return false;
	}
	
	if (fcntl(fd, F_SETFL, flags | O_NONBLOCK) == -1)
	{
		LOG_CORE("fail fcntl(F_SETFL) err(%d) : %s\n", errno, strerror(errno));
		return false;
	}
#endif
...
bool Socket::ReuseAddrs()
{
	...
	{
		LOG_CORE("fail setsockopt(SO_REUSEADDR) err(%d) : %s\n", errno, strerror(errno));
		return false;
	}
...
#endif
	{
		LOG_CORE("fail setsockopt(SO_LINGER) err(%d) : %s\n", errno, strerror(errno));
		return false;
	}
...
#endif
	{
		LOG_CORE("fail setsockopt(TCP_NODELAY) err(%d) : %s\n", errno, strerror(errno));
		return false;
	}
...
bool Socket::InheritProperties(SOCKET parent)
{
	...
	{
		LOG_CORE("fail setsockopt(SO_UPDATE_ACCEPT_CONTEXT) err(%d) : %s\n", errno, strerror(errno));
		return false;
	}
...
#endif //_WINDOWS
	{
		LOG_CORE("fail setsockopt(SO_SNDBUF) err(%d) : %s\n", errno, strerror(errno));
		return -1;
	}
...
#endif //_WINDOWS
	{
		LOG_CORE("fail setsockopt(SO_REVBUF) err(%d) : %s\n", errno, strerror(errno));
		return -1;
	}
